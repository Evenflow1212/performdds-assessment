...
function parsePL(text){
  const RE_TOTAL_INCOME=/^\s*Total\s+Income\s+([\d,]+\.\d{2})\s*$/i;
  const RE_TOTAL_EXPENSE=/^\s*Total\s+Expense\s+([\d,]+\.\d{2})\s*$/i;
  const RE_NET_INCOME=/^\s*Net\s+(?:Ordinary\s+)?Income\s+([\-\(]?[\d,]+\.\d{2}\)?)\s*$/i;
  function parseAmt(s){s=s.trim().replace(/,/g,'');const neg=s.startsWith('(')&&s.endsWith(')');s=s.replace(/[()]/g,'');return neg?-parseFloat(s):parseFloat(s);}
  let totalIncome=null,totalExpense=null,netIncome=null;
  for(const line of text.split('\n')){
    let m;
    if(totalIncome===null&&(m=RE_TOTAL_INCOME.exec(line)))totalIncome=parseAmt(m[1]);
    if(totalExpense===null&&(m=RE_TOTAL_EXPENSE.exec(line)))totalExpense=parseAmt(m[1]);
    if((m=RE_NET_INCOME.exec(line)))netIncome=parseAmt(m[1]);
  }
  if(totalIncome===null)throw new Error('P&L: Total Income not found');
  if(totalExpense===null)throw new Error('P&L: Total Expense not found');
  if(netIncome===null)throw new Error('P&L: Net Income not found');
  const computedNet=Math.round((totalIncome-totalExpense)*100)/100;
  if(Math.abs(computedNet-netIncome)>0.02)throw new Error('P&L validation FAILED: '+totalIncome+'-'+totalExpense+'='+computedNet+' but PDF says '+netIncome);
  return{totalIncome,totalExpense,netIncome,netCollections:Math.round(totalIncome*100)/100};
}
...
async function buildXlsx(raw,groups,pl,months,name){
  ...
  s('B2','PRODUCTION OVERVIEW');s('B4','practice');if(name)s('D4',name);
  s('B5','number of months reviewed');s('D5',months);s('E5','total production');s('G5',Math.round(tot*100)/100);
  ...
  s('B25','arestin (4381)');s('D25',g(groups,'hyg_arestin'));f('E25','=D25/D5');s('F25',raw.D4381?.avg||0);f('G25','=D25*F25');
  ...
  s('B33','inlays & onlays');s('D33',ioQ);f('E33','=D33/D5');s('F33',ioQ>0?Math.round(ioT/ioQ*100)/100:0);f('G33','=D33*F33');
  ...
  if(pl){
    ...
    try{wsFO.getCell('B4').value='practice';wsFO.getCell('D4').value=name||'';wsFO.getCell('D25').value=pl.netCollections/months;wsFO.getCell('D26').value=pl.netCollections/months;wsFO.getCell('D28').value={formula:'=IFERROR(D26/D27,0)'};}catch(e){}
    try{wsBud.getCell('B7').value='COLLECTION';wsBud.getCell('C7').value=pl.netCollections/months;}catch(e){}
  }
...
exports.handler = async function(event) {
  ...
  try{
    const PROD_PROMPT='Eaglesoft Procedures by Provider report. Return each ADA code as CODE|QTY|TOTAL (e.g. 00120|340|420594.36). Combine all providers. Data lines only, no headers or explanations.';
    const PL_PROMPT='QuickBooks Profit & Loss PDF. Extract the raw text exactly as it appears, preserving indentation and line breaks so that "Total Income", "Total Expense", and "Net Income" lines are clearly identifiable. Return the full extracted text with no reformatting.';
    const calls=[callClaude(productionBase64,PROD_PROMPT,KEY)];
    if(plBase64)calls.push(callClaude(plBase64,PL_PROMPT,KEY));
    const results=await Promise.all(calls);
    const prodText=results[0];
    const plText=results[1]||'';
    const raw=parseProd(prodText);
    const groups=agg(raw);
    let pl=null;
    if(plBase64&&plText){
      try{pl=parsePL(plText);}catch(e){console.error('PL parse error:',e.message);}
    }
